...
class DSARF(nn.Module):
    ...
    def fit(self, data, epoch_num = 500, return_stats=False):
        ...
        if return_stats:
            return dsarf, {'loss':loss_value/len(train_loader.dataset), 
                           'NRMSE':NRMSE['NRMSE_pred']}
        else:
            return dsarf
    ...
    class DSARF_(nn.Module):
        ...
        def short_predict(self, s=None):
            ...
            # p(w_t|w_{t-1}, s_t) = Normal(w_loc, w_scale)
        ...
        def long_predict(self, steps, s = None):
            ...
            for i in range(steps):
                p_z_mu, p_z_sig = self.dsarf.trans(z_t_1) # S*N*factor_dim
                p_s = self.dsarf.strans(s_t_1,
