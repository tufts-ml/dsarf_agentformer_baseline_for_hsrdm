...
def train(epoch):
    ...
    while not generator.is_epoch_end():
        ...
        if generator.index - last_generator_index > cfg.print_freq:
            ...
            if 'mse' in train_loss_meter.keys():
                curr_perf_dict = {'epoch':epoch,
                                  'iter' : generator.index,
                                  'mse' : train_loss_meter['mse'].avg,
                                  'total_loss' : train_loss_meter['total_loss'].avg}
            else:
                curr_perf_dict = {'epoch':epoch,
                                  'iter' : generator.index,
                                  'recon' : train_loss_meter['recon'].avg,
                                  'total_loss' : train_loss_meter['total_loss'].avg}
            
            # save performance to monitor later
            perf_dict_list.append(curr_perf_dict)
            perf_df = pd.DataFrame(perf_dict_list)
            save_dir = os.path.join(cfg.result_dir, current_time)
            
            
            if not os.path.exists(save_dir):
                # Create the directory
                os.makedirs(save_dir)
            perf_df.to_csv(os.path.join(save_dir, 'perf.csv'), index=False)
                        
            
            if generator.batch_size is None:
                logging(args.cfg, epoch, cfg.num_epochs, generator.index, generator.num_total_samples, ep, seq, frame, losses_str, log)
            else:
                logging(args.cfg, epoch, cfg.num_epochs, generator.index, generator.batch_size, ep, seq, frame, losses_str, log)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--set_type', type=str, default='small')
    ...
    """ data """
    generator = data_generator(cfg, log, split='train', phase='training', set_type=args.set_type)
